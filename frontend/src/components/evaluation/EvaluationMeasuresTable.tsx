...
export function EvaluationMeasuresTable({
  measures,
  variant = 'full',
}: EvaluationMeasuresTableProps) {
  const { formatMessage } = useIntl();

  function measureDescriptionText(measure: EvaluationMeasure): string {
    return measure.measureTypeCode
      ? getMeasureTypeDescription(measure.measureTypeCode, formatMessage)
      : '';
  }
              ...
              <tr key={m.id}>
                <td className="col-text">
                  {formatMeasureTypeName(formatMessage, {
                    code: m.measureTypeCode,
                    name: m.measureTypeName,
                  })}
                </td>
        ...
        <tbody>
          {measures.map((m) => (
            <tr key={m.id}>
              ...
              <td className="col-text">
                {m.ratingComment ?? formatMessage({ id: 'common.emptyValue' })}
              </td>
            </tr>
          ))}
        </tbody>
