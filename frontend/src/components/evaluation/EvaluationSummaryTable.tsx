...
export function EvaluationSummaryTable({
  ...
  const resolvedPendingActionLabel = pendingActionLabel ?? formatMessage({ id: 'evaluation.rateAndSubmit' });
  ...
  return (
    <>
      <div className="table-panel">
        ...
        <table className={`table table--hover table--evaluations${isPendingTab ? ' table--evaluations--with-actions' : ' table--clickable'}`}>
        <thead>
          <tr>
            <th className="col-text">{formatMessage({ id: 'admin.employees' })}</th>
            {showEvaluator && <th className="col-text">{formatMessage({ id: 'admin.evaluators' })}</th>}
        ...
        <tbody>
          {evaluations.map((ev) => (
            <tr
              key={ev.id}
              onClick={isPendingTab ? undefined : () => openEvaluation(ev.id)}
              onKeyDown={isPendingTab ? undefined : (event) => handleRowKeyDown(event, ev.id)}
            ...
            >
              <td className="cell-primary col-text">
                ...
                {showNewBadge && isNewForController(ev) && (
                  <span className="badge badge-new" style={{ marginLeft: '0.5rem' }}>{formatMessage({ id: 'evaluation.new' })}</span>
                )}
              </td>
              {showEvaluator && <td className="cell-muted col-text">{ev.evaluatorFullName}</td>}
...
export function EvaluationSummaryTableCard({
  ...
  const { formatMessage } = useIntl();
  const emptyCopy = activeTab && emptyStateMap
