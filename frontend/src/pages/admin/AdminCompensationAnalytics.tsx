...
export function AdminCompensationAnalytics() {
  ...
  const [chartType, setChartType] = useState<CompensationAnalyticsChartType>('shareDistribution');
  ...
  const selectedChartLabel = useMemo(
    () => {
      const option = COMPENSATION_CHART_OPTIONS.find((entry) => entry.value === chartType);
  ...
  const loadAnalytics = useCallback(async (orgId: string, selectedYear: string, selectedChart: CompensationAnalyticsChartType) => {
  ...
  const hasData = analytics && (analytics.buckets.some((b) => b.count > 0) || analytics.series.length > 0);
    ...
    <div className="card compensation-analytics">
      <div className="compensation-analytics__top">
        ...
        <div className="form-row">
          ...
          >
            <option value={ALL_ORG_VALUE}>{formatMessage({ id: 'common.allOrganization' })}</option>
            {orgUnits.map((unit) => (
        ...
        <div className="form-row">
          <label htmlFor="analytics-year">{formatMessage({ id: 'common.year' })}</label>
          <select id="analytics-year" value={year} onChange={(e) => setYear(e.target.value)}>
            {yearOptions.map((y) => (
        ...
        <div className="form-row">
          <label htmlFor="analytics-chart">{formatMessage({ id: 'common.chartType' })}</label>
          <select
            id="analytics-chart"
            value={chartType}
            onChange={(e) => setChartType(e.target.value as CompensationAnalyticsChartType)}
          >
            {COMPENSATION_CHART_OPTIONS.map((option) => (
              <option key={option.value} value={option.value}>{formatMessage({ id: option.labelKey as never })}</option>
            ))}
          </select>
        </div>
        </div>
