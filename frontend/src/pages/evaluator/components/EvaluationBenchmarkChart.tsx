...
export function EvaluationBenchmarkChart({
      ...
      return [
        {
          ...
          employeeAverage: averageNonNull(
            filteredQuarters.map((q) => q.employeeAverage),
          ),
          organizationUnitAverage: averageNonNull(
            filteredQuarters.map((q) => q.organizationUnitAverage),
          ),
          jobPositionAverage: averageNonNull(
            filteredQuarters.map((q) => q.jobPositionAverage),
          ),
        },
      ];
    }

    return [...filteredQuarters].reverse().map((point) => ({
  ...
  const maxValue = 5;
  const groupWidth =
    chartData.length > 0 ? chartWidth / chartData.length : chartWidth;
  const barWidth = isAggregateView
    ? Math.min(72, (chartWidth - 80) / 3)
    : Math.min(18, (groupWidth - 16) / 3);
  const aggregatePeriodLabel = formatMessage({
    id: 'charts.benchmarkAllYears',
  });
    ...
    <div className="benchmark-chart">
        ...
        <div className="benchmark-chart__canvas-wrap">
          {tooltip && (
            <div
              ...
              role="tooltip"
            >
              <span className="benchmark-chart__tooltip-period">
                {tooltip.period}
              </span>
              <strong>
                  ...
                  {SERIES_KEYS.map((series, seriesIndex) => {
                    ...
                    return (
                      <rect
                        ...
                        onMouseEnter={(e) =>
                          showTooltip(e, seriesLabel, value, period)
                        }
                        onMouseMove={(e) =>
                          showTooltip(e, seriesLabel, value, period)
                        }
      ...
      <div className="benchmark-chart__footer">
        <div className="benchmark-chart__footer-inner">
          <div
            className="benchmark-chart__legend"
          ...
          >
            {SERIES_KEYS.map((series) => (
              <span key={series.key} className="benchmark-chart__legend-item">
                <span
                  className="benchmark-chart__swatch"
          ...
          <label
            className="benchmark-chart__filter"
            htmlFor="chart-year-filter"
          >
