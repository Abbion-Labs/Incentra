...
export function toTextDrafts(items: { description: string; sortOrder: number }[]): TextItemDraft[] {
  const valid = items.filter((item) => item.description.trim());
  if (valid.length === 0) {
    return [{ description: '', sortOrder: 0 }];
  }
  return valid.map((item) => ({ description: item.description, sortOrder: item.sortOrder }));
}

export function isGoalsPlanningComplete(
  evaluation: Pick<EvaluationSummary, 'goalsPlanningComplete' | 'goalCount'> | EvaluationDetail,
): boolean {
  ...
  if ('goals' in evaluation) {
    return evaluation.goals.some((goal) => goal.description.trim())
...
export function hasValidPlanningDraft(
  ...
  return goals.some((goal) => goal.description.trim())
